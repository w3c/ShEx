...
\end{gendef}
The function below is used to execute the rules that are referenced by \zc{GroupRule}s,
...
evalArcRule~ec~g~ar = \\
\t1 \IF \# ar.min = 1 \land \# sg = 0 \land  value~ ar.min = 0 \\
\t2 \THEN \nomatch \\
\t1 \ELSE \IF \# sg = 0 \\
\t2 \THEN \dunno \\
\t1 \ELSE \IF (\# ar.min = 1 \land \# sg < value~ ar.min) \lor \\
\t3 (\# ar.max = 1 \land \# sg > value~ ar.max) \\
\t2 \THEN \fail \\
