...
\t2 evalValueSet~(valueSet \entryFor os)~n \\
\t1 \ELSE \IF os \in \ran osWild \THEN \\
\t2 evalValueWild~(osWild \entryFor os)~n \\
\t1 \ELSE \\
\t2 evalValueReference~ec~(valueRef \entryFor os)~n
\end{gendef}
...
\where
   \forall mvs: \power MatchValue; n: RDFTerm @ evalValueSet~mvs~n = \\
\t1	\IF \exists mv: mvs @ \\
\t2 ((mv \in \ran mviri \land (iri \entryFor n) = mviri \entryFor mv) \lor \\
\t2  (mv \in \ran mviris \land (iri \entryFor n) \in IRIstemRange ~ (mviris \entryFor mv) ) \lor \\
\t2  (mvlit \entryFor mv = literal \entryFor n)) \\
\t1 \THEN \pass \\
\t1 \ELSE \fail
\end{gendef}
...
\where
   \forall mvs: \power MatchValue; n: RDFTerm @ evalValueWild~mvs~n = \\
\t1 \IF evalValueSet~ mvs~ n = \pass \THEN \fail \ELSE \pass
\end{gendef}
...
\where
	\forall ec: EvalContext; vr: Label; n: RDFTerm @ \\ evalValueReference~ec~vr~n = \\
...
\t2 evalValueSet~(subjectSet \entryFor ss)~n \\
\t1 \ELSE \IF ss \in \ran ssWild \THEN \\
\t2 evalValueWild~(ssWild \entryFor ss)~n \\
\t1 \ELSE \\
\t2 evalValueReference~ec~(subjectRef \entryFor ss)~n
\end{gendef}
...
\begin{itemize}
\item \zc{g} - the \zc{Graph} that passed the associated Rule.
\item \zc{ec} - the evaluation context
\item \zc{as} - the set of \zc{Actions} associated with the associated \zc{GroupRule} or \zc{ArcRule}
