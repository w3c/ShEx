...
Starting with the definitions in \textit{RDF 1.1 Concepts and Abstract Syntax}\cite{RDF11}:
...
The $ShEx$ language treats \textbf{IRI}s and \textbf{blank nodes} as primitive types, which are reflected as the $Z$ basic types:
\begin{zed}
[IRI, BlankNode]
\end{zed}

The $ShEx$ language can express constraints on both the type and content of RDF literals:
...
\end{itemize}
\end{quote}
This can be modelled by $String$ and $LanguageTag$ as basic types:
\begin{zed}
[String, LanguageTag]
\end{zed}
and using them in the definition the two flavours of $RDFLiteral$,  plain literal and typed literal::
\begin{zed}
...
The $ShEx$ language requires the following basic functions:
...
A Shape Expression \zc{Schema} is a collection of labelled shapes where exactly one shape in the collection is identified as the outermost or ``starting'' shape. In addition, any shape 
that is referenced within the \zc{Schema} is also itself a member of the \zc{Schema} Formally:

\begin{schema}{Schema}
   shapes : Label \pfun Rule \\
   start : Label
\where
   start \in \dom shapes \\
\forall r: \ran shapes @ \\
\t1 (r \in \ran group \implies (group \entryFor r).rule \in \dom shapes) \land \\
\t1 (r \in \ran and \implies \ran (and \entryFor r) \subseteq \dom shapes) \land \\
\t1 (r \in \ran xor \implies \ran (xor \entryFor r) \subseteq \dom shapes) \land \\ 
\t1 (r \in \ran arc \land (arc \entryFor r).o \in \ran osShape \implies \\
\t2 (osShape \entryFor (arc \entryFor r).o) \in \dom shapes) \land \\
\t1 (r \in \ran rarc \land (rarc \entryFor r).s \in \ran ssShape \implies \\
\t2 (ssShape \entryFor (rarc \entryFor r).s) \in \dom shapes)
\end{schema}


While $ShEx$ implementations define a rule \zc{Label} as being either an \zc{IRI} or
a \zc{BlankNode}, the type of \zc{Label} does not impact the evaluation semantics, allowing it to be treated as a basic type:
\begin{zed}
[Label]
\end{zed}

The validity of a given RDF \zc{Graph} is determined by taking the \zc{start} \zc{Label} of a
$ShEx$ \zc{Schema} and a reference \zc{IRI} and evaluating the validity of the corresponding \zc{shape} against the supplied graph. 

Formally, the \zc{evaluate} function takes a \zc{Schema}, a \zc{Graph} and a reference \zc{IRI}
and, if the \zc{Rule} associated with the \zc{start} \zc{Label} in the \zc{Schema}, 
...
\t1 ec.graph = g \land ec.schema = s @ \\
   evaluate ~s ~g ~i = \\
\t1 \IF evalRule ~ec ~ (iri~ i)~ (s.shapes~ s.start) \in \{\nomatch, \pass\} \\
...
\where
   \forall ec: EvalContext; l:Label @ l \in \dom ec.schema.shapes
\end{gendef}
...
\t1 sg = evalPredicateFilter~ar.p~(triplesForSubject~ s~ ec.graph) @ \\
evalArcRule~ec~s~ar = \\
\t1 \IF evalCardinality~sg~ar.min~ar.max \neq \pass \\
\t2 \THEN evalCardinality~sg~ar.min~ar.max \\
\t1 \ELSE \IF evalObjectSpecification~ec~ar.o~sg \neq \pass \\
\t2 \THEN evalObjectSpecification~ec~ar.o~sg \\
\t1 \ELSE \\
...
\end{itemize}
\begin{zed}
IRIorStem ::= iosIRI \ldata IRI \rdata | stem \ldata IRIstem \rdata \\
...
\t1 \IF e \in \ran iosIRI \THEN \{t : g | iri \entryFor t.p = iosIRI \entryFor e \} \\
...
\t1 og = evalPredicateFilter~rar.p~(triplesForObject~ o~ ec.graph) @ \\
evalRevArcRule~ec~o~rar = \\
\t1 \IF evalCardinality~og~rar.min~rar.max \neq \pass \\
\t2 \THEN evalCardinality~og~rar.min~rar.max \\
\t1 \ELSE \IF evalSubjectSpecification~ec~rar.s~og \neq \pass \\
\t2 \THEN evalSubjectSpecification~ec~rar.s~og \\
\t1 \ELSE \\
...
\t1 \IF \forall t: g @ evalSubjectSpecificationTriple~ec~ss~t.s = \pass \THEN \pass \\
\t1 \ELSE \fail
\end{gendef}
\begin{gendef}
